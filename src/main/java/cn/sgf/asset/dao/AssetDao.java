...
public interface AssetDao extends JpaRepository<AssetDO, Long>,JpaSpecificationExecutor<AssetDO> {
	...
	AssetDO findByCodeAndUsingOrgan(String code,SysOrganDO usingOrgan);
	...
	@Modifying
	@Query("update AssetDO u set u.status = :status,u.usingOrgan=:usingOrgan,usingTime=:usingTime,usingUser=:usingUser where u.id = :id")
	void editStatus(Integer status,SysOrganDO usingOrgan,Date usingTime,String usingUser, Long id);
